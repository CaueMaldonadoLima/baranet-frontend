...
// Marcas (telas 16–18). Marca não é papel de Pessoa.
export async function GET(request: NextRequest) {
  const opticaId = resolveOpticaId(request.cookies.get(OPTICA_COOKIE)?.value);
  const qs = request.nextUrl.search;

  const upstream = await baranetFetch(`/v1/oticas/${opticaId}/brands${qs}`);
...
export async function POST(request: NextRequest) {
  ...
  const upstream = await baranetFetch(`/v1/oticas/${opticaId}/brands`, {
    method: "POST",
    body,
  });
